...
(()=>{
  ...
  .ac-support-sub{color:#8495b1;font-size:12px;margin:4px 0 15px}
  .ac-tabs{display:grid;grid-template-columns:1fr 1fr;gap:7px}.ac-tab{padding:11px;border:1px solid #274568;border-radius:12px;background:#0b172b;color:#8495b1;font-weight:900}.ac-tab.active{border-color:#00eaff;color:#f3f8ff}
  ...
  .ac-status{min-height:18px;text-align:center;color:#8495b1;font-size:11px;margin-top:9px}.ac-status.ok{color:#63ffb0}.ac-status.error{color:#ff718f}
  .ac-success{display:none;margin-top:10px;padding:12px;border:1px solid #236847;border-radius:13px;background:#071b16;color:#63ffb0;font-size:11px}
  .ac-note{color:#607390;font-size:10px;line-height:1.6;margin-top:10px}
  `;
  ...
  function mount(){
    ...
    bg.innerHTML=`<section class="ac-support" role="dialog" aria-modal="true" aria-label="Acerola Support and Pro">
      ...
      <div id="acDonate" class="ac-panel active">
        <div class="ac-hero"><b>Help keep the Core growing</b><span>Choose a preset or enter your own amount.</span></div>
        ...
        <input id="acEmail" class="ac-field" type="email" autocomplete="email" placeholder="Email for payment receipt">
        <button id="acDonatePay" class="ac-pay">⚡ SUPPORT ACEROLA</button>
      </div>
      <div id="acPro" class="ac-panel">
        <div class="ac-hero"><b>Acerola Pro · ₦1,000 one-time</b><span>Optional Pro status for premium features as they are released.</span></div>
        <div class="ac-note">◆ Pro identity · ⚡ Premium modes · 🎨 Premium UI · ☁ Future cloud-first features</div>
        <input id="acProEmail" class="ac-field" type="email" autocomplete="email" placeholder="Email for payment receipt">
        <button id="acProPay" class="ac-pay pro">◆ UNLOCK ACEROLA PRO</button>
      </div>
      <div id="acStatus" class="ac-status">Secure Paystack checkout · NGN</div>
      <div id="acSuccess" class="ac-success"></div>
      <div class="ac-note">Payments are initialized and verified by the Acerola server.</div>
    </section>`;
    document.body.appendChild(bg);
    $('#acSupportClose').onclick=close;
    bg.addEventListener('click',e=>{if(e.target===bg)close()});
    document.addEventListener('keydown',e=>{if(e.key==='Escape'&&bg.classList.contains('open'))close()});
    document.querySelectorAll('[data-ac-tab]').forEach(b=>b.onclick=()=>switchTab(b.dataset.acTab));
    document.querySelectorAll('[data-ac-amount]').forEach(b=>b.onclick=()=>selectAmount(Number(b.dataset.acAmount)));
    $('#acCustom').oninput=()=>{if($('#acCustom').value)document.querySelectorAll('[data-ac-amount]').forEach(b=>b.classList.remove('active'))};
    $('#acDonatePay').onclick=()=>start({email:$('#acEmail').value.trim(),amount:amount(),product:'donation',button:$('#acDonatePay')});
    $('#acProPay').onclick=()=>start({email:$('#acProEmail').value.trim(),amount:1000,product:'premium',button:$('#acProPay')});
  }
  function open(){mount();$('#acSupportBg').classList.add('open');document.body.style.overflow='hidden'}
  function close(){const x=$('#acSupportBg');if(x)x.classList.remove('open');document.body.style.overflow=''}
  ...
  function selectAmount(n){selected=n;$('#acCustom').value='';document.querySelectorAll('[data-ac-amount]').forEach(b=>b.classList.toggle('active',Number(b.dataset.acAmount)===n))}
  function amount(){const raw=$('#acCustom').value.trim();const n=raw?Number(raw):selected;return Number.isFinite(n)&&n>=100&&n<=1000000?Math.round(n):null}
  function setStatus(t,type=''){const x=$('#acStatus');x.textContent=t;x.className='ac-status'+(type?' '+type:'')}
  ...
  async function verify(reference,amt,product){const d=await post(VERIFY_URL,{reference,amount:amt});if(!d.verified)throw new Error(d.error||d.reason||'Payment could not be verified');$('#acSuccess').style.display='block';$('#acSuccess').textContent=product==='premium'?'◆ Acerola Pro unlocked and verified.':'✓ Support payment verified. Thank you for helping Acerola grow.';if(product==='premium')localStorage.setItem('acerola-premium-unlocked','1');setStatus('Payment verified successfully.','ok');return true}
  ...
  mount();addCoreLink();
  const obs=new MutationObserver(addCoreLink);obs.observe(document.body,{childList:true,subtree:true});
  window.AcerolaSupport={open,close};
})();
